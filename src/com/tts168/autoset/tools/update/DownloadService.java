...
public class DownloadService extends Service {
	...
	public static String apk_name="larksmart.apk";//项目安装包版本名称
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 3:
				...
				// 取消通知
				mNotificationManager.cancel(NOTIFY_ID);
				NotifyDialog.SimpleAlertDialog(MyApplication.getInstance().getCur_Activity(),"下载失败，找不到下载的文件！！" );
				stopSelf();// 停掉服务自身
				//ToastTools.long_Toast(mContext, "找不到下载文件");
				break;
			case 4:
				app.setDownload(false);
				// 这里是找不到下载的文件
				// 取消通知
				mNotificationManager.cancel(NOTIFY_ID);
				NotifyDialog.SimpleAlertDialog(MyApplication.getInstance().getCur_Activity(),"下载失败，URL异常（MalformedURLException）！！" );
				stopSelf();// 停掉服务自身
				//ToastTools.long_Toast(mContext, "找不到下载文件");
				break;
			case 1:
	...
	private void setUpNotification() {
		...
		contentView.setTextViewText(R.id.name, "百灵智能 LarkSmart.apk 正在下载...");
		// 指定个性化视图
