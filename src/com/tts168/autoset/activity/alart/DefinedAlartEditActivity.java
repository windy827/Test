 ...
 *
 */
public class DefinedAlartEditActivity extends MyBaseActivity implements OnClickListener{
	TitleView titleView;//标题栏，如果想相应点击事件，需要在titleSave方法里面去实现
	public static final String TITLE="自定义闹铃";
	
	public static final String ActivityName="DefinedAlartEditActivity";
	View view_title;//标题栏
	AlartCloseAndDelView alartCloseAndDelView;//关闭和删除界面
	AlartTitleView alartTitleView;//标题界面
	DefinedAlartView definedAlartView;//自定义主要内容的界面
	/**
	 * 当前界面是否为添加界面
	 */
	boolean isadd;
	...
	public void staticFindView() {
		...
		alartTitleView = new AlartTitleView(this,AlartTitleView.TEXTLEN_MAX20);
		alartTitleView.setEitTextContent(AlartTools.alarmEntity.getTitle());
		
		alartCloseAndDelView=new AlartCloseAndDelView(this);	
		definedAlartView=new DefinedAlartView(this);
		definedAlartView.setIsAdd(isadd, AlartTools.alarmEntity,index);
			...
			PreventForceClick.isShowToast=true;
				...
				case R.id.iv_acticity_awakealart_isopen:
					...
					if(AlartTools.alarmEntity.getIs_valid()==AlarmEntity.ISVALID_YES){
						...
						closeOrOpenSucess="关闭闹铃成功"+"已为您关闭了"+MyTools.getDoubQuot(AlartTools.alarmEntity.getClock().substring(0, 5))+"的自定义闹铃";
					}else{
						//AlartTools.alarmEntity.setIs_valid(AlarmEntity.ISVALID_YES);
						isOpen=true; 
						closeOrOpenFaild="开启闹铃失败";
						closeOrOpenSucess="开启闹铃成功"+"已为您开启了"+MyTools.getDoubQuot(AlartTools.alarmEntity.getClock().substring(0, 5))+"的自定义闹铃";
					}
					...
					if (alartTitleView.et_title.getText().length() > 0) {
						...
						NotifyDialog.SimpleAlertDialog(this,
